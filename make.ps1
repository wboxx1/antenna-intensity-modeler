...
<#
.Description
Make file for antenna-intensity-modeler
#>
...
#>
function lint() {
    poetry run flake8 src\antenna_intensity_modeler tests
}
...
#>
function docs(){
    if (Test-Path ".\docs\antenna_intensity_modeler.rst") { Remove-Item ".\docs\antenna_intensity_modeler.rst" -Force }
    if (Test-Path ".\docs\modules.rst") { Remove-Item ".\docs\modules.rst" -Force }
    poetry run sphinx-apidoc -o docs\ .\src\antenna_intensity_modeler
...
function servedocs() {
    docs
    poetry run watchmedo shell-command -p "*.rst" -c "cd .\docs && make html" -R -D
}
